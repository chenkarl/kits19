...
def overlay(volume_ims, segmentation_ims, segmentation, alpha):
    ...
    overlayed = np.where(
        ...
        0#np.round(volume_ims).astype(np.uint8)
    )
    return overlayed
...
def visualize(cid, destination, hu_min=DEFAULT_HU_MIN, hu_max=DEFAULT_HU_MAX,
    ...
    plane_opts = ["axial", "coronal", "sagittal", "divide"]
    ...
    if plane == plane_opts[3]:
        for i in range(vol_ims.shape[0]):
            fpath_vol = out_path / ("{:05d}_vol.png".format(i))
            imwrite(str(fpath_vol), vol_ims[i])
            fpath_seg = out_path / ("{:05d}_seg.png".format(i))
            imwrite(str(fpath_seg), seg_ims[i])
